...
#ifndef SHARKFUND_KEY_H
#define SHARKFUND_KEY_H
...
public:
	...
	unsigned int size() const {
		unsigned int len = GetLen(vch[0]);
		 if(len != 33) //only use 33 for Sharkfund sys
			 return 0;
		return len;
	}
 ...
 *  A CTxDestination is the internal data type encoded in a CSharkfundAddress
 */
