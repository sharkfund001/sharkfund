...
private:
	CSharkfundAddress *addr;
public:
	CDacrsAddressVisitor(CSharkfundAddress *addrIn) :
			addr(addrIn) {
	}
...
bool CSharkfundAddress::Set(const CTxDestination &dest) {
...
//bool CSharkfundAddress::GetRegID(CRegID &Regid) const {
//
////	vector<unsigned char> vid;
////	vid.push_back(CBaseParams::ACC_ADDRESS);
...
bool CSharkfundSecret::SetString(const char* pszSecret) {
	return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CSharkfundSecret::SetString(const string& strSecret) {
