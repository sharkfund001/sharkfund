...
int LogPrintStr(const char* category, const string &str) {
	...
	if (SysCfg().IsPrintToFile()) {
		...
		pathDebug = GetDataDir() / file;   // /home/share/bille/SHARKFUND_test/regtest/INFO.log
...
#else
	const char* pszModule = "Sharkfund";
#endif
	if (pex)
		return strprintf(
...
boost::filesystem::path GetDefaultDataDir() {
	...
	// Windows < Vista: C:\Documents and Settings\Username\Application Data\Sharkfund
	// Windows >= Vista: C:\Users\Username\AppData\Roaming\Sharkfund
	// Mac: ~/Library/Application Support/Sharkfund
	// Unix: ~/.Sharkfund
#ifdef WIN32
	// Windows
	return GetSpecialFolderPath(CSIDL_APPDATA) / "Sharkfund";
#else
	fs::path pathRet;
...
boost::filesystem::path GetConfigFile() {
	boost::filesystem::path pathConfigFile(CBaseParams::GetArg("-conf", "sharkfund.conf"));
			...
			if (!fDone) {
				...
				if (!fMatch) {
					...
					string strMessage = _("Warning: Please check that your computer's date and time "
							"are correct! If your clock is wrong Sharkfund will not work properly.");
