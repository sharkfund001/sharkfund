...
// Copyright (c) 2014-2015 The Sharkfund developers
...
//	LogPrint("INFO","proof-of-work found  \n  hash: %s  \ntarget: %s\n", hash.GetHex(), hashTarget.GetHex());
	...
	// Found a solution
	{
		...
		if (pblock->GetHashPrevBlock() != chainActive.Tip()->GetBlockHash())
			return ERRORMSG("SharkfundMiner : generated block is stale");
...
//		}
		...
		if (!ProcessBlock(state, NULL, pblock))
			return ERRORMSG("SharkfundMiner : ProcessBlock, block not accepted");
	}
...
void static SharkfundMiner(CWallet *pwallet,int targetConter) {
	LogPrint("INFO","Miner started\n");

	SetThreadPriority(THREAD_PRIORITY_LOWEST);
	RenameThread("Sharkfund-miner");
	...
	if (!CheckIsHaveMinerKey()) {
			LogPrint("INFO", "SharkfundMiner  terminated\n");
...
void GenerateSharkfundBlock(bool fGenerate, CWallet* pwallet, int targetHigh) {
...
//		assert(0);
	...
	minerThreads->create_thread(boost::bind(&SharkfundMiner, pwallet,targetHigh));
