...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"Sharkfundaddress\"   (string, required) The Sharkfund address for the private key\n"
            "\nResult:\n"
    ...
    CSharkfundAddress address;
    if (!address.SetString(strAddress))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Sharkfund address");
    CKeyID keyID;
    ...
    if (!pwalletMain->GetKey(keyID, minerkey,true))
           throw JSONRPCError(RPC_WALLET_ERROR, "Private key for address " + strAddress + " is not known");
    Object reply;
    	reply.push_back(Pair("privkey", CSharkfundSecret(vchSecret).ToString()));
    if(minerkey.ToString() != vchSecret.ToString())
    	reply.push_back(Pair("minerkey", CSharkfundSecret(minerkey).ToString()));
    else
...
Value dumpwallet(const Array& params, bool fHelp) {
	...
	Object reply;
	reply.push_back(Pair("created by Sharkfund", CLIENT_BUILD + CLIENT_DATE));
