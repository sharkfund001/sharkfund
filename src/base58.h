...
//
#ifndef SHARKFUND_BASE58_H
#define SHARKFUND_BASE58_H
...
/** base58-encoded Sharkfund addresses.
 * Public-key-hash-addresses have version 0 (or 111 testnet).
 * The data vector contains RIPEMD160(SHA256(pubkey)), where pubkey is the serialized public key.
 * Script-hash-addresses have version 5 (or 196 testnet).
 * The data vector contains RIPEMD160(SHA256(cscript)), where cscript is the serialized redemption script.
 */
class CSharkfundAddress : public CBase58Data {
public:
    ...
    CSharkfundAddress() {}
    CSharkfundAddress(const CTxDestination &dest) { Set(dest); }
    CSharkfundAddress(const string& strAddress) { SetString(strAddress); }
    CSharkfundAddress(const char* pszAddress) { SetString(pszAddress); }
 ...
 */
class CSharkfundSecret : public CBase58Data
{
public:
    ...
    CSharkfundSecret(const CKey& vchSecret) { SetKey(vchSecret); }
    CSharkfundSecret() {}
};

template<typename K, int Size, CBaseParams::Base58Type Type> class CSharkfundExtKeyBase : public CBase58Data
{
public:
    ...
    CSharkfundExtKeyBase(const K &key) {
        SetKey(key);
    }

    CSharkfundExtKeyBase() {}
};

typedef CSharkfundExtKeyBase<CExtKey, 74, CBaseParams::EXT_SECRET_KEY> CSharkfundExtKey;
typedef CSharkfundExtKeyBase<CExtPubKey, 74, CBaseParams::EXT_PUBLIC_KEY> CSharkfundExtPubKey;

#endif // SHARKFUND_BASE58_H
