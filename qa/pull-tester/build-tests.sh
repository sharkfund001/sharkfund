...
if [ -d "$OUT_DIR" -a -w "$OUT_DIR" ]; then
  set +e
  # Windows:
  cp /d/project/sharkfund/win32-build/src/bitcoind.exe $OUT_DIR/bitcoind.exe
  cp /d/project/sharkfund/win32-build/src/test/test_bitcoin.exe $OUT_DIR/test_bitcoin.exe
  cp /d/project/sharkfund/win32-build/src/qt/bitcoind-qt.exe $OUT_DIR/bitcoin-qt.exe
  # Linux:
  cp /d/project/sharkfund/linux-build/src/bitcoind $OUT_DIR/bitcoind
  cp /d/project/sharkfund/linux-build/src/test/test_bitcoin $OUT_DIR/test_bitcoin
  cp /d/project/sharkfund/linux-build/src/qt/bitcoind-qt $OUT_DIR/bitcoin-qt
  set -e
fi

# Run unit tests and blockchain-tester on Linux:
cd /d/project/sharkfund/linux-build
make check

# Run RPC integration test on Linux:
/d/project/sharkfund/qa/rpc-tests/wallet.sh /d/project/sharkfund/linux-build/src
/d/project/sharkfund/qa/rpc-tests/listtransactions.py --srcdir /d/project/sharkfund/linux-build/src
# Clean up cache/ directory that the python regression tests create
rm -rf cache

if [ $RUN_EXPENSIVE_TESTS = 1 ]; then
  # Run unit tests and blockchain-tester on Windows:
  cd /d/project/sharkfund/win32-build
  make check
fi
